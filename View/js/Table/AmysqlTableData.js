...
var TableDataObject;
var PageObject;
var ScreenSum_number = 30;
var ScreenSum = C(CreatesSelect(['30|30', '60|60', '100|100'], ScreenSum_number), {'id':'ScreenSum'});	// 一屏显示条数
...
var SetOrderBy = function (obj)
{
	...
	if(temp_location == -1)
	{
		...
		sql = sql + ' ORDER BY `' + obj.table + '`.`' + obj.name + '` ' + OrderByType + limit;
	}
	else
	{
		...
		if(sql_order_by_field == obj.table + '.' + obj.name) OrderByType = sql_order_by_type == 'DESC' ? 'ASC' : 'DESC';
	    sql = trim(sql1) + ' ORDER BY `' + obj.table + '`.`' + obj.name + '` ' + OrderByType + limit;
...
var TableData = function ()
{
	...
	this.OnceShow = parseInt(ScreenSum.value);		// 一屏显示条数
	this.RightButtonRow = null;						// 当前右键记录
	this._IfShow = false;

	// Dom
	if(TableDataObject) TableMainObject.ContantBlock.removeChild(TableDataObject.TableDataMainBlock);	
	C(this.TableDataMainBlock, 'In', [ C('div', {'id':'ListPageOffsetTop'}), C(C('div', {'id':'ListData'}), 'In', [C('div', {'id':'PageListTop'}), this.list]) ])
	C(TableMainObject.ContantBlock, 'In', this.TableDataMainBlock);
			...
			for (var i = 0; i < ThItemSum; ++i)
			{
				...
				with(this)
				{
					(function(obj)
					{
						...
						// 排序
						obj.ATag.onclick = function ()
						{
							SqlSubmitFormObject.sql_post.value = SetOrderBy(obj);	// 给sql 设置OrderBy
				...
				for (SEkey in Item)
				{
					if(Item[SEkey].textarea.length > 0)			// 哪些行有编辑
					{
						...
						for (var SEi = 0; SEi < ThItemSum; ++SEi)		 
						{
							...
							var table_info_tmp = table_info(ThItem[SEi].name);	 // 取字段信息
		...
		for (; ; ++k)
		{
			...
			for (var Thi = 0; Thi < ThItemSum; ++Thi)
			{
				...
				if(CanEdit)
				    ...
				    var field_div = C('div', 'In', HTMLEnCode(field_val) + '&nbsp;');
				...
				if(CanEdit && table_info_tmp.COLUMN_TYPE != 'text') td.noWrap = 'noWrap';	// 非txt类型不换行
				...
				with(this)
				{
					...
					Item[i].edit.onclick = function (type)
					{
						...
						if(Item[this.key].textarea.length == 0 && type != 'CancelEdit')
						{
							for (var Thi = 0; Thi < ThItemSum; ++Thi)
							{
								var table_info_tmp = table_info(ThItem[Thi].name);		// 正确取字段信息
								...
								if(table_info_tmp.COLUMN_TYPE.indexOf('blob') != -1)	// blob类型编辑
								{
									...
									Item[this.key].textarea[Thi].MaxSizeDom = C('i', 'In', 'Upload MaxSize:' + Item[this.key].textarea[Thi].MaxSize);
									Item[this.key].textarea[Thi].UpBlobDom = C('span', {'id':'UpBlobId' + this.key + '' + Thi}, {'display':'block'});			 // 上传进度
									Item[this.key].textarea[Thi].UpButtonDom = C('span', {'id':'UpBlobButtonId' + this.key + '' + Thi}, {'display':'block'});	 // 上传按钮
								...
								else
								{
									...
									if(table_info_tmp.COLUMN_TYPE == 'text') 
										var offsetHeight = Item[this.key].btr.getElementsByTagName('div')[Thi].scrollHeight * Item[this.key].btr.getElementsByTagName('div')[Thi].scrollWidth / 380 + 20;
									
									var textarea_height = (table_info_tmp.COLUMN_TYPE != 'text') ? Item[this.key].btr.getElementsByTagName('div')[Thi].scrollHeight - 4 + 'px' : offsetHeight + 'px' ;
									var textarea_width = (table_info_tmp.COLUMN_TYPE != 'text') ? Item[this.key].btr.getElementsByTagName('div')[Thi].scrollWidth - 12 + 'px'  : '380px';
								...
								{
									var url = 'index.php?c=upload&a=UpBlobData&FieldName=' + encodeURIComponent(ThItem[Thi].name) + '&TableName=' + encodeURIComponent(TableName) + '&DatabaseName=' + encodeURIComponent(DatabaseName) + '&where=' + encodeURIComponent(Item[this.key].del.Where.join(' AND '));
									UploadInit(Item[this.key].textarea[Thi].id, url, Item[this.key].textarea[Thi].MaxSize, '*.*', '1', '1',Item[this.key].textarea[Thi].UpButtonDom.id, Item[this.key].textarea[Thi].UpBlobDom.id, '', 'FieldData'+Item[this.key].textarea[Thi].id);
									// 唯一id, 上传URL, 最大大小, 允许类型, 数量, 排队数量, 按钮id, 进度id, 进度组id, 返回数据处理id
								}
							}
							for (var i = 0; i < Item[this.key].btr.getElementsByTagName('div').length ; ++i)
							{
								Item[this.key].btr.getElementsByTagName('div')[i].style.display = 'none';
							}
						}
						else if(type != 'edit')
						{
