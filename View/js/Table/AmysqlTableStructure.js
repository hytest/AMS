...
var Table = function ()
{
	...
	if(TableObject) TableMainObject.ContantBlock.removeChild(TableObject.list);	
	C(TableMainObject.ContantBlock, 'In', this.list);
		...
		with(this)
		{
			(function (obj)
			{
				...
				obj.PrimeKey.onclick = function ()
				{
					...
					for (key in Item)
					{
						if (Item[key].input.checked) temp_arr.push(Item[key].Field.replace(/`/g, '``'));
					}
					if(temp_arr.length < 1) return false;
