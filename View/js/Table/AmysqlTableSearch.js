...
var TableSearch = function ()
{
	...
	C(TableMainObject.ContantBlock, 'In', C(this.TableSearchForm, 'In', [this.Slist, this.TSSql, this.TSPage]));
	...
	this.show = function ()
	{
		...
		for (var i = 0; i < ItemSum; ++i)
		{
			...
			SelectArr['text'] = SelectArr['varchar'] = CreatesSelect(StrOperators);

			var SOtd = C('td');
			this.SItem[i].SelectOption =  (typeof(SelectArr[this.SItem[i].DATA_TYPE]) == 'object') ? SelectArr[this.SItem[i].DATA_TYPE] : SelectArr['default'];
